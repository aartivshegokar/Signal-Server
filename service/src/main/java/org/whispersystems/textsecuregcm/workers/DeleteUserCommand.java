...
public class DeleteUserCommand extends EnvironmentCommand<WhisperServerConfiguration> {
      ...
      throws Exception
  {
    try {
      ...
      ReplicatedJedisPool cacheClient     = new RedisClientFactory("main_cache_delete_command", configuration.getCacheConfiguration().getUrl(), configuration.getCacheConfiguration().getReplicaUrls(), configuration.getCacheConfiguration().getCircuitBreakerConfiguration(), configuration.getCacheConfiguration().getPassword()).getRedisClientPool();
